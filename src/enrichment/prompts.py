...
ENRICHMENT_USER_PROMPT_TEMPLATE = """Analyze the following text and return a JSON object with these exact keys:
- "is_valid_review": boolean (true/false). Set to false if this text is a general quote, random spam, news headline, survey response, political discussion, or otherwise NOT a genuine user review of a quick-commerce/delivery experience on Blinkit.
- "themes": list of 1-3 short theme labels (e.g., "Category Trust", "Delivery Delay", "Quality Issue"). (Empty list if not a valid review).
- "sentiment": one of "Positive", "Neutral", "Negative". If a review uses sarcasm (e.g., "Great job delivering expired milk!"), tag it as Negative. Assign sentiment specifically *relative to the target platform* (Blinkit) being analyzed. (If not a valid review, use "Neutral").
- "segment": list of BEHAVIORAL user segments inferred from the review text. Segments MUST predict a specific purchase-category need or an exploration pattern. 
  ...
  Return an empty list if no valid behavioral segment can be inferred or if not a valid review.
